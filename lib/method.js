 ...
 */
Method.prototype.execute = function(server, requestParams, callback, meta) {
  var options = this.options;
  var handler = this.getHandler();
  var params = this._getHandlerParams(requestParams);

  if(options.collect) {
    return handler.call(server, params, callback, meta);
  }
  ...
  return handler.apply(server, _.flatten([params, callback, meta]));
};
